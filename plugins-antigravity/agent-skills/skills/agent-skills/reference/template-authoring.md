...
Prefer portable helpers over literal platform syntax. Consult the sibling reference `cli-and-helpers.md` for the supported helper surface and CLI grammar. Raw `it.platform` conditionals are an escape hatch for cases the helper model does not cover; a new repeated dialect difference belongs in the platform definitions and helper reference instead.
