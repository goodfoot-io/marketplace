...
**Configuring the log file:** Logging is configured at runtime, not at build time. Set the `AGENT_HOOKS_LOG_FILE` environment variable to write JSON-line logs to a file, or configure programmatically with `new Logger({ logFilePath })` / `new Logger({ logEnvVar })`. See [Logging & Debugging](@plugins-antigravity/agent-hooks/skills/codex/reference/logging.md).
...
```bash
npx @goodfoot/agent-hooks --agent codex --scaffold ./packages/my-codex-hooks --hooks PreToolUse,PostToolUse -o ../../plugins-antigravity/my-plugin/.codex/hooks.json
```
See [Installation: Scaffolding for Monorepos](@plugins-antigravity/agent-hooks/skills/codex/reference/installation.md).
...
```bash
npx -y @goodfoot/agent-hooks --agent codex -i "src/**/*.ts" -o "../../plugins-antigravity/my-plugin/hooks/hooks.json" --plugin-root
```
See [Monorepo Integration](@plugins-antigravity/agent-hooks/skills/codex/reference/installation.md).
...
*   **[Installation & Setup](@plugins-antigravity/agent-hooks/skills/codex/reference/installation.md)**: Setup guide (Scaffolding vs Manual).
*   **[All 10 Hook Inputs](@plugins-antigravity/agent-hooks/skills/codex/reference/input-types.md)**: Field shapes per event, with `tool_input` narrowing.
*   **[All 10 Output Builders](@plugins-antigravity/agent-hooks/skills/codex/reference/output-builders.md)**: Factories, builders, and option types.
*   **[Porting from Bash](@plugins-antigravity/agent-hooks/skills/codex/reference/porting.md)**: Migration guide.
*   **[Logging & Debugging](@plugins-antigravity/agent-hooks/skills/codex/reference/logging.md)**: How to see what's happening.
*   **[Environment](@plugins-antigravity/agent-hooks/skills/codex/reference/environment.md)**: Hook context and runtime env.
