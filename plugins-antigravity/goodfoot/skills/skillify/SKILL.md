...
Convert a task just completed in the current session into a reusable skill — either a new skill or an edit to an existing one. Every skill produced is written for Antigravity as its sole reader: capture only what a capable Antigravity could not already infer, generalized so the skill works in any repository.
...
- **Trivially inferable workflow**: A capable Antigravity would already do it correctly — stop.
...
- **Inclusions** — The non-obvious insights to keep versus the steps Antigravity would already perform correctly.
...
- **Omit** what a capable Antigravity with no memory of this session would already do correctly. **Keep** what it would not: non-obvious decision logic, ordering that matters, safety gates, and failure modes discovered during the work.
...
- Note how Antigravity will know each consequential step succeeded, and flag any irreversible action for a human checkpoint.
...
- No instruction restates what Antigravity would already do, and each consequential step carries a success signal.
