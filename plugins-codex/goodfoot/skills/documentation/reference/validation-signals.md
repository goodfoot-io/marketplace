...
|---|---|---|---|
| Baseline audit | corpus counts: size, stale, orphan, duplicate, unclassified | `git ls-files`, `git grep`, `git log -1 --format=%cs`, `wiki check`, `git span drift` | counts scope the work, not its priority |
| Card sorting | co-change clusters; contributor language | history mining, `git span list <path>`, commit/issue/PR text | a cluster is a candidate grouping, confirm by tree-test |
