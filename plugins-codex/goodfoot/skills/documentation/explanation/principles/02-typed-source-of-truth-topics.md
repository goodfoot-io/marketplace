...
Code and config own behavior; the layered source of truth assigns every other fact an owner (`../foundations.md` §1). The concerns: **topic identity** (what deserves its own page versus a section), **documentation mode and page genre**, the four **topic types** (concept, task, reference, troubleshooting — plus glossary), and **canonicality** (resolving competing pages to one owner). Separate why from how structurally; record decision history in commits/PRs/ADRs; record a load-bearing unenforced coupling as a span.
...
Related: the layered owner model `../foundations.md`; hubs route but never own these facts `03-hubs-orientation-and-routing.md`; record couplings `../../reference/tools/git-span.md`.
