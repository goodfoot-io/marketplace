...
npx @goodfoot/agent-hooks --agent claude-code --scaffold ./packages/my-hooks \
  ...
  -o ../../plugins-codex/my-plugin/hooks/hooks.json
```

This generates the build script:
```json
"build": "npx -y @goodfoot/agent-hooks --agent claude-code -i \"src/**/*.ts\" -o \"../../plugins-codex/my-plugin/hooks/hooks.json\""
```
...
npm run build
# Check: plugins-codex/my-plugin/hooks/hooks.json should exist
cat ../../plugins-codex/my-plugin/hooks/hooks.json | jq .
```
