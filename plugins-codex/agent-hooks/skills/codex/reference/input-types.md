...
This document describes the typed hook inputs in `@goodfoot/agent-hooks/codex`. Shapes are derived from the wire schemas under `third_party/reference/codex/codex-rs/hooks/schema/generated/*.input.schema.json`. When the docs and the schemas disagree, the schemas win.
...
`@goodfoot/agent-hooks/claude-code` ships `isBashTool`, `isWriteTool`, `getFilePath`, etc., because Claude Code's tool set is fixed and known to the SDK. Codex tools (shell, MCP-provided tools, host extensions) are open-ended, so `@goodfoot/agent-hooks/codex` deliberately does not ship per-tool predicates. Define them locally in your hook code or in a shared module.
