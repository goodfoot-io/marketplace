...
**Configuring the log file:** Logging is configured at runtime, not at build time. Set the `AGENT_HOOKS_LOG_FILE` environment variable to write JSON-line logs to a file, or configure programmatically with `new Logger({ logFilePath })` / `new Logger({ logEnvVar })`. See [Logging & Debugging](codex/logging.md).
...
```
See [Installation: Scaffolding for Monorepos](codex/installation.md).
...
```
See [Monorepo Integration](codex/installation.md).
...
*   **[Installation & Setup](codex/installation.md)**: Setup guide (Scaffolding vs Manual).
*   **[All 10 Hook Inputs](codex/input-types.md)**: Field shapes per event, with `tool_input` narrowing.
*   **[All 10 Output Builders](codex/output-builders.md)**: Factories, builders, and option types.
*   **[Porting from Bash](codex/porting.md)**: Migration guide.
*   **[Logging & Debugging](codex/logging.md)**: How to see what's happening.
*   **[Environment](codex/environment.md)**: Hook context and runtime env.
