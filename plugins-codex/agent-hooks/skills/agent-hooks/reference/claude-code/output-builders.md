...
- `.md` imports work by default because `agent-hooks` enables `.md=text`.
...
```typescript
import { postToolUseHook, postToolUseOutput, isBashTool } from '@goodfoot/agent-hooks/claude-code';
...
```typescript
import { postToolUseHook, postToolUseOutput, getFilePath, isTsFile } from '@goodfoot/agent-hooks/claude-code';
