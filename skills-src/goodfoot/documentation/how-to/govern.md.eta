...
Gate `wiki check` and `git span drift` in a git hook or CI so doc↔code drift fails the build; this is the repo's advantage over a generic wiki. Anchor load-bearing claims as fragment links and spans. Keep the gate side-effect-free: use the read-only `wiki check` / `git span drift` (they fail without mutating); never run a `--fix` form or an auto-staging step in a commit hook — it can sweep unrelated files into the commit. Prefer a verify-only hook or CI; if a hook must mutate, commit narrowly by explicit path and inspect the staged set first.
...
Related: why govern `../explanation/principles/08-validate-and-iterate.md`; the validation that precedes it `validate.md`; the loop `procedure.md`; drift gates `../reference/tools/git-span.md`, `../reference/tools/wiki.md`.
