...
| Check | Owned by |
|---|---|
| Add a scope line to every touched page that passes the value test — skip where the title already orients | `strengthen-scent.md` |
| Add a navigational index to documents over 500 lines — a TOC, or a curated version/year index for a changelog | `build-hubs.md` |
| Add cross-references between related documents; grep the target to confirm the return path | `add-paths.md` |
| Prefer structural changes (indexing, cross-refs, reorganization) over cosmetic ones; don't pad a pass with low-value scope lines | `design-around-intent.md` |
