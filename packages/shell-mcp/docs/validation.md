...
| TypeScript typecheck | passed | `yarn workspace @goodfoot/shell-mcp run typecheck`, no diagnostics |
| Production build | passed | `yarn workspace @goodfoot/shell-mcp run build` |
| Biome | passed | `yarn workspace @goodfoot/shell-mcp run lint`: 23 files checked, no diagnostics and no fixes applied |
...
| --- | --- | --- |
| TypeScript typecheck | passed | `yarn workspace @goodfoot/shell-mcp run typecheck`, no diagnostics |
| Production build | passed | `yarn workspace @goodfoot/shell-mcp run build`; the emitted `build/dist/src/main.js` keeps the `#!/usr/bin/env node` shebang the manifest's `bin` depends on |
| Biome | passed | `yarn workspace @goodfoot/shell-mcp run lint`: 25 files checked, no diagnostics and no fixes applied |
...
| TypeScript typecheck | passed | `yarn workspace @goodfoot/shell-mcp run typecheck`, no diagnostics |
| Production build | passed | `yarn workspace @goodfoot/shell-mcp run build` through `prepack`; the emitted `build/dist/src/main.js` keeps its `#!/usr/bin/env node` shebang and its executable mode |
| Biome | passed | `yarn workspace @goodfoot/shell-mcp run lint`: 25 files checked, no diagnostics and no fixes applied |
