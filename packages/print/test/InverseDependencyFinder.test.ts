...
describe("InverseDependencyFinder", () => {
  ...
  describe("execute", () => {
    ...
    it("should handle non-existent files gracefully with warning", async () => {
      ...
      try {
        ...
        expect(result).toEqual([]);
        expect(warnSpy).toHaveBeenCalledWith("Warning: No files found matching the input glob patterns.");
      } finally {
        warnSpy.mockRestore();
      }
    });
