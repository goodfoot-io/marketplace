...
class TypeAnalyzer {
  ...
  /**
   * Safely get the text of a node using the source file.
   * getText() can fail if parent chain is not set up properly.
   */
  private getNodeText(node: ts.Node, sourceFile: ts.SourceFile): string {
    try {
      return node.getText(sourceFile);
    } catch {
      // Fallback: manually extract text from source file
  ...
  private analyzeInterface(node: ts.InterfaceDeclaration, sourceFile: ts.SourceFile): void {
      ...
      for (const clause of node.heritageClauses) {
        if (clause.token === ts.SyntaxKind.ExtendsKeyword) {
          for (const type of clause.types) {
            extendsList.push(this.getNodeText(type.expression, sourceFile));
          }
        }
      }
    }
    ...
    node.members.forEach((member) => {
      if (ts.isPropertySignature(member) || ts.isMethodSignature(member)) {
        const propName = member.name ? this.getNodeText(member.name, sourceFile) : "anonymous";
        const propType = member.type ? `: ${this.getNodeText(member.type, sourceFile)}` : "";
  ...
  private analyzeClass(node: ts.ClassDeclaration, sourceFile: ts.SourceFile): void {
    ...
    if (node.heritageClauses) {
      for (const clause of node.heritageClauses) {
        if (clause.token === ts.SyntaxKind.ExtendsKeyword) {
          ...
          for (const type of clause.types) {
            implementsList.push(this.getNodeText(type.expression, sourceFile));
          }
        }
      }
    }
    ...
    node.members.forEach((member) => {
      if (ts.isPropertyDeclaration(member) || ts.isMethodDeclaration(member)) {
        const memberName = member.name ? this.getNodeText(member.name, sourceFile) : "anonymous";
  ...
  private analyzeEnum(node: ts.EnumDeclaration, sourceFile: ts.SourceFile): void {
    ...
    node.members.forEach((member) => {
      const memberName = member.name ? this.getNodeText(member.name, sourceFile) : "anonymous";
      const memberValue = member.initializer ? ` = ${this.getNodeText(member.initializer, sourceFile)}` : "";
  ...
  private analyzeFunction(node: ts.FunctionDeclaration | ts.MethodDeclaration, sourceFile: ts.SourceFile): void {
    const name = node.name ? this.getNodeText(node.name, sourceFile) : "anonymous";
    const { line } = sourceFile.getLineAndCharacterOfPosition(node.getStart(sourceFile));
    ...
    node.parameters.forEach((param) => {
      const paramName = this.getNodeText(param.name, sourceFile);
      const paramType = param.type ? `: ${this.getNodeText(param.type, sourceFile)}` : "";
      parameters.push(`${paramName}${paramType}`);
    });

    // Get return type
    if (node.type) {
      returnType = this.getNodeText(node.type, sourceFile);
    }
