...
/**
 * Files whose pinned pre-migration body has been **superseded**, not migrated.
 *
 * The equivalence contract asks one question: did templating change content it
 * should have preserved? That question stops applying when a later card
 * deliberately rewrites a file because the pinned text was found to be wrong.
 * A `BODY_TRANSFORMS` entry is the right tool while the rewrite is a bounded
 * set of substitution sites — see `agentSkillsAntigravityPolicy`, which
 * supersedes five paragraphs. It is the wrong tool once the rewrite is
 * wholesale: enumerating the whole document as replacements transcribes the
 * new output into the fixture under the guise of reconstructing it, the same
 * tautology `helperReferenceTable` refuses.
 *
 * So a wholesale rewrite leaves this contract instead, and lands where the hub
 * SKILL.md files already sit: correctness owned by the freshness and lint
 * gates, plus whatever check the rewriting card brought with it. Each entry
 * must name that check, so an exit from this gate cannot be silent.
 *
 * Entries are permanent. A superseded file never returns to the pinned corpus,
 * because the blob it would be compared against is the text that was wrong.
 */
const SUPERSEDED: Record<string, Record<string, string>> = {
  "agent-hooks": {
    "antigravity/SKILL.md":
      "The pinned body describes Antigravity as an unbuildable target and lists wire fields " +
      "(snake_case `tool_name`/`session_id`, a `deny_unless_prior_grant` decision) that the host " +
      "never sends and never accepted — they were derived by analogy from Claude Code and Codex " +
      "before any Antigravity reference existed. The rewrite is transcribed from the host's own " +
      "hook reference, pinned at packages/agent-hooks/src/agents/antigravity/CONTRACT.md and " +
      "verified against agy 1.1.22. Covered by tests/conformance/antigravity.test.ts, whose " +
      "payloads are that contract's shapes.",
  },
};
...
describe("templated-plugin equivalence (pre-migration blob vs generated output)", () => {
  ...
  for (const plugin of fixturePlugins) {
    ...
    describe(`${plugin.name}`, () => {
      ...
      // A superseded file leaves the equivalence contract, so the reason it
      // left has to stay legible and has to name a real fixture entry. Both
      // are asserted here rather than trusted to review.
      it("every superseded entry names a pinned file and records why", () => {
        const pinned = new Set(fixture.files.map((entry) => entry.path));
        for (const [path, reason] of Object.entries(SUPERSEDED[plugin.name] ?? {})) {
          expect(pinned.has(path), `${path} is superseded but absent from the pinned corpus`).toBe(true);
          expect(reason.length, `${path} is superseded without a recorded reason`).toBeGreaterThan(80);
        }
      });
      ...
      for (const target of plugin.targets) {
        ...
        it(`renders ${target.platform} tree (${target.path}) as the pinned corpus, parsed-frontmatter-plus-body`, () => {
          for (const entry of fixture.files) {
            if (SUPERSEDED[plugin.name]?.[entry.path] !== undefined) continue;
