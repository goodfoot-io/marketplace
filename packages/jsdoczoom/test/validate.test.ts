...
describe('validate', () => {
  ...
  it('fails file with 1 @summary tag (even with free-text)', () => {
    ...
    expect(result.files[0].issues).toContain('Found 1 @summary tag(s), minimum is 2');
  });
  ...
  it('fails file with 0 @summary tags', () => {
    ...
    expect(result.files).toHaveLength(1);
    expect(result.files[0].passed).toBe(false);
    expect(result.files[0].issues).toContain('Found 0 @summary tag(s), minimum is 2');
  });
  ...
  it('rejects @depth suffix with INVALID_DEPTH error', () => {
    ...
    try {
      ...
      expect((e as JsdocError).message).toBe('Validation mode does not support @depth');
    }
  });
  ...
  it('works with glob selectors', () => {
    ...
    const twoSummaries = result.files.find(f => f.path.includes('two-summaries.ts'));
