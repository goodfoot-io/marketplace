...
import type { SelectorInfo, OutputItem } from '../src/types.js';
...
describe('drilldown with barrels', () => {
  ...
  it('children appear at their shallowest summary level', () => {
    ...
    expect(isOutputItem(helperEntry!)).toBe(true);
    if (isOutputItem(helperEntry!)) {
    ...
    expect(isOutputItem(utilsEntry!)).toBe(true);
    if (isOutputItem(utilsEntry!)) {
  ...
  it('barrel with zero children returns empty array at transition', () => {
    ...
    expect(() =>
      drilldown(globSelector('**/*.ts', 2), zeroChildDir),
    ).toThrow();
  });
  ...
  it('nested barrel child appears as regular item with direct-path id (when revealed)', () => {
    ...
    expect(isOutputItem(subBarrel!)).toBe(true);
    if (isOutputItem(subBarrel!)) {
