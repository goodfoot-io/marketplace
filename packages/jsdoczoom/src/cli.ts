...
function handleValidationExitCode(result: ValidationResult): void {
  if (result.summary.failed > 0) {
    process.stderr.write(
      JSON.stringify(
        new JsdocError('VALIDATION_FAILED', `${result.summary.failed} file(s) failed validation`).toJSON(),
      ) + '\n',
    );
    process.exitCode = 2;
  }
...
function isDirectRun(): boolean {
  ...
  try {
    ...
    return import.meta.url.endsWith(scriptPath) || import.meta.url.endsWith('/cli.js');
  } catch {
    return false;
  }
}
