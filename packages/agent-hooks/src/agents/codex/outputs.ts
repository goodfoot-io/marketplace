...
/**
 * The Codex block signal, carried into the new package as a subclass of the
 * shared core {@link ../core/transport.ts|HookBlockError} so `drive()`
 * classifies it **before** consulting `unexpectedError` policy — carrying an
 * ordering the source runtime already implemented (`runtime.ts` tested
 * `instanceof BlockError` ahead of the policy branch). Public surface is
 * preserved: `name`, `reason`, and `instanceof BlockError` semantics are
 * unchanged, and `instanceof HookBlockError` now also holds.
 */
export class BlockError extends HookBlockError {
