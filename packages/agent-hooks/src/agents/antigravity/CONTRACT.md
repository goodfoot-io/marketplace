# Antigravity hook contract (pinned)

Everything this directory implements is transcribed from the host's own hook
reference, not inferred from Claude Code or Codex.

**Provenance.** The reference ships inside the `agy` binary as the embedded
asset `assets/external/skills/agy-customizations/docs/hooks.md`. It is not
published on the web and is not present in any file the installer writes to
disk; it was recovered by extracting printable strings from the binary. The
copy below is verbatim.

**Verified against the host.** `agy plugin install` reports
`hooks: 1 processed` for a plugin carrying `hooks.json` at its **root**, and
`hooks: skipped (not found)` for the same plugin with a Claude Code style
`hooks/hooks.json`. The card's open question — which layout the host reads —
is answered by that A/B, and the embedded reference agrees.

**Payload verified live.** A compiled `PreToolUse` hook installed into a
throwaway `HOME` received exactly the fields below from `agy -p`, confirming
the reference's camelCase encoding and the absence of any event-name field:

```json
{
  "artifactDirectoryPath": "…/antigravity-cli/brain/<conversationId>",
  "conversationId": "e256b375-bfe6-4316-89fc-e1d829114a68",
  "modelName": "gemini-3.7-flash-high",
  "stepIdx": 2,
  "toolCall": { "name": "run_command", "args": { "CommandLine": "echo hello-from-agy", "Cwd": "…" } },
  "transcriptPath": "…/transcript_full.jsonl",
  "workspacePaths": []
}
```

The reply `{"decision":"deny","reason":"<sentinel>"}` came back to the model as
`tool call denied by pre-tool hook: <sentinel>`, so the decision channel is
payload-only as documented. Note `workspacePaths` can be empty; it is an array
either way.

**Host version.** `agy` 1.1.22.

Re-verify this document against the binary when `agy` changes minor version,
and record the version above with the result.
