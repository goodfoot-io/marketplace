 ...
 * hook runtime logging, equivalent to AGENT_HOOKS_LOG_FILE.
 */
...
function runCli(
  ...
  const args = ["--agent", "claude-code", "-i", inputPattern, "-o", outputPath];
  if (options.logPath) {
...
describe("E2E: --log CLI Parameter", () => {
  ...
  describe("Runtime env var override", () => {
    it("runtime AGENT_HOOKS_LOG_FILE overrides --log compiled path", () => {
      ...
      const execResult = spawnSync("node", [commandPath], {
        ...
        encoding: "utf-8",
        timeout: 5000,
        env: { ...process.env, AGENT_HOOKS_LOG_FILE: runtimeLogPath },
      });
    ...
    it("works without --log when only AGENT_HOOKS_LOG_FILE is set at runtime", () => {
      ...
      const execResult = spawnSync("node", [commandPath], {
        ...
        encoding: "utf-8",
        timeout: 5000,
        env: { ...process.env, AGENT_HOOKS_LOG_FILE: logPath },
      });
  ...
  describe("Log File Path Resolution", () => {
    it("resolves relative --log paths to absolute paths in compiled hook", () => {
      ...
      expect(content).toMatch(/AGENT_HOOKS_LOG_FILE.*=.*relative-test\.log/);
      ...
      const pathMatch = content.match(/process\.env\['AGENT_HOOKS_LOG_FILE'\]\s*=\s*("(?:[^"\\]|\\.)*")/);
  ...
  describe("Compiled Hook Content", () => {
    it("injects AGENT_HOOKS_LOG_FILE assignment when --log is used", () => {
      ...
      // Banner should set AGENT_HOOKS_LOG_FILE to the hardcoded path
      expect(content).toContain("AGENT_HOOKS_LOG_FILE");
    ...
    it("does not inject log path assignment when neither --log nor --log-env-var is used", () => {
      ...
      const logFileAssignment = /process\.env\[['"]AGENT_HOOKS_LOG_FILE['"]\]\s*=\s*['"]/;
      const logEnvVarAssignment = /process\.env\[['"]AGENT_HOOKS_LOG_ENV_VAR['"]\]\s*=\s*['"]/;
      expect(content).not.toMatch(logFileAssignment);
      expect(content).not.toMatch(logEnvVarAssignment);
    });

    it("injects AGENT_HOOKS_LOG_ENV_VAR when --log-env-var is used", () => {
      ...
      expect(content).toContain("AGENT_HOOKS_LOG_ENV_VAR");
