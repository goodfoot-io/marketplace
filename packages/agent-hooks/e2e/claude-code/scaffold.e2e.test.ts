...
describe("E2E: Scaffold Command", () => {
  ...
  describe("File Structure Generation", () => {
    ...
    it("generates correct package.json content", () => {
      ...
      const deps = packageJson.dependencies as Record<string, string>;
      expect(deps["@goodfoot/agent-hooks"]).toBeDefined();
    });
    ...
    it("generates hook templates with correct imports", () => {
      ...
      expect(preToolUseContent).toContain('from "@goodfoot/agent-hooks/claude-code"');
    ...
    it("generates CLAUDE.md with skill loading instruction", () => {
      ...
      expect(claudeMdContent).toBe("Load the `agent-hooks:claude-code` skill immediately if it is available.\n");
    });
  ...
  describe("Error Handling", () => {
    ...
    it("missing --hooks argument fails with error", () => {
      ...
      const result = runScaffoldCli(["--agent", "claude-code", "--scaffold", testDir, "-o", "dist/hooks.json"]);
    ...
    it("missing -o argument fails with error", () => {
      ...
      const result = runScaffoldCli(["--agent", "claude-code", "--scaffold", testDir, "--hooks", "Stop"]);
