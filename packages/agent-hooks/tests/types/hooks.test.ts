...
import type { HookConfig, HookContext, HookHandler } from "../../src/agents/claude-code/index.js";
import {
  notificationHook,
...
import type { PreToolUseOutput } from "../../src/agents/claude-code/outputs.js";
import type { PreToolUseInput } from "../../src/agents/claude-code/types.js";
  ...
  describe("accept HookConfig", () => {
    for (const { name, factory } of factories) {
      it(`${name} accepts config with matcher and timeout`, () => {
        ...
        // Structural factory view for the generic loop: every per-event
        // factory accepts matcher/timeout config and returns metadata carrying
        // both back (excluded events narrow only the policy field, which this
        // config does not set).
        const anyFactory = factory as (
          config: { matcher?: string; timeout?: number },
          handler: () => PreToolUseOutput,
        ) => { matcher?: string; timeout?: number };
        const hook = anyFactory(config, () => preToolUseOutput({}));
