 ...
 *
 * - **Dual-path topology** — the checkout's `node_modules/@goodfoot/agent-hooks`
 ...
 *   OWN source tree: a physical `packages/agent-hooks` copy, with the
 *   checkout's `node_modules/@goodfoot/agent-hooks` a directory symlink back
 *   into it (`../../packages/agent-hooks`). This is the workspace-monorepo
...
describe("compileHook install-topology reproducibility", () => {
  ...
  function createSinglePathCheckout(): string {
    ...
    const pkgDir = path.join(checkoutDir, "node_modules", "@goodfoot", "agent-hooks");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        // `@goodfoot/agent-hooks` resolves into the physical copy.
        exports: { ".": "./src/index.ts", "./codex": "./src/agents/codex/index.ts" },
      }),
      "utf-8",
    );
  ...
  function createDualPathCheckout(singlePathCheckout: string): string {
    ...
    fs.symlinkSync(
      path.join(singlePathCheckout, "node_modules", "@goodfoot", "agent-hooks"),
      path.join(checkoutDir, "node_modules", "@goodfoot", "agent-hooks"),
      "dir",
    );
    return checkoutDir;
  }
   ...
   * source tree — a physical `packages/agent-hooks` copy, with the checkout's
  ...
  function createSiblingTreeCheckout(): string {
    ...
    const pkgDir = path.join(checkoutDir, "packages", "agent-hooks");
    fs.mkdirSync(pkgDir, { recursive: true });
    fs.cpSync(PACKAGE_SRC_DIR, path.join(pkgDir, "src"), { recursive: true });
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
    ...
    fs.mkdirSync(path.join(checkoutDir, "node_modules", "@goodfoot"), { recursive: true });
    fs.symlinkSync(
      path.join("..", "..", "packages", "agent-hooks"),
      path.join(checkoutDir, "node_modules", "@goodfoot", "agent-hooks"),
      "dir",
    );
    return checkoutDir;
  ...
  function writeHook(checkoutDir: string): string {
    ...
    fs.writeFileSync(
      hookPath,
      [
        `import { stopHook, PACKAGE_NAME, EXIT_CODES } from '@goodfoot/agent-hooks/codex';`,
        ``,
  ...
  it("produces byte-identical compiled output regardless of install topology", () => {
    ...
    expect(singlePathEntry).toContain(
      `import { execute } from "./node_modules/@goodfoot/agent-hooks/src/agents/codex/transport`,
    );
