 ...
 *    `./node_modules/@goodfoot/agent-hooks/src/agents/claude-code/transport.js`, so the
 ...
 *    directory, with the checkout's `node_modules/@goodfoot/agent-hooks`
...
describe("compileHook install-topology reproducibility", () => {
  ...
  beforeEach(() => {
    ...
    installPhysicalCliPackage(path.join(checkoutS, "node_modules", "@goodfoot", "agent-hooks"));
    ...
    fs.symlinkSync(
      path.join(checkoutS, "node_modules", "@goodfoot", "agent-hooks"),
    ...
    installPhysicalCliPackage(path.join(checkoutE, "packages", "agent-hooks"));
    fs.symlinkSync(
      path.join("..", "..", "packages", "agent-hooks"),
      path.join(checkoutE, "node_modules", "@goodfoot", "agent-hooks"),
      "dir",
    );
  });
  ...
  function installPhysicalCliPackage(cliPkgDir: string): void {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        // `@goodfoot/agent-hooks` resolves into the checkout install.
  ...
  it("anchors the single-path wrapper's runtime import inside the checkout's node_modules", () => {
    ...
    // `./node_modules/@goodfoot/agent-hooks/src/agents/claude-code/transport.js` — anchored
