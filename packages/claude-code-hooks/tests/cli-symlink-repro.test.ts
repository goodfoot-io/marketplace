...
describe("compileHook symlinked dependency reproducibility", () => {
  ...
  function installCliPackageIntoSharedInstall(): void {
    const cliPkgDir = path.join(sharedNodeModulesDir, "@goodfoot", "claude-code-hooks");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        // `@goodfoot/claude-code-hooks` resolves into the shared install.
  ...
  it("embeds a checkout-depth-independent runtime import in the entry wrapper's sourcesContent", () => {
    ...
    expect(shallowEntry).toContain(`import { execute } from './node_modules/@goodfoot/claude-code-hooks/src/runtime`);
  }, 60000);
});
