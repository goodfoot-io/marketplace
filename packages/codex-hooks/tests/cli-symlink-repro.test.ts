 ...
 * symlinked dependency — the codex-hooks twin of
 * `packages/claude-code-hooks/tests/cli-symlink-repro.test.ts`.
 *
 ...
 *    the path was computed by JavaScript, not esbuild. Fixed by
 *    `symlinkVisiblePath`, which re-roots the runtime path through the
 *    checkout's own node_modules symlink.
 *
...
describe("compileHook symlinked dependency reproducibility", () => {
  ...
  beforeEach(() => {
    baseDir = fs.mkdtempSync(path.join(os.tmpdir(), "codex-cli-symlink-repro-"));
  ...
  function installCliPackageIntoSharedInstall(): void {
    const cliPkgDir = path.join(sharedNodeModulesDir, "@goodfoot", "codex-hooks");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        // `@goodfoot/codex-hooks` resolves into the shared install.
        exports: { ".": "./src/index.ts" },
      }),
      "utf-8",
    );
    // The shared install carries only this package; its npm dependencies
    // (esbuild, glob, typescript) come from the workspace install.
  ...
  function createCheckout(checkoutDir: string): string {
    ...
    fs.writeFileSync(
      hookPath,
      [
        `import { stopHook, PACKAGE_NAME } from '@goodfoot/codex-hooks';`,
        ``,
        `export default stopHook({}, (input) => {`,
        `  return { name: PACKAGE_NAME, input };`,
        `});`,
        ``,
      ].join("\n"),
      "utf-8",
  ...
  it("embeds a checkout-depth-independent runtime import in the entry wrapper's sourcesContent", () => {
    ...
    // (codex-hooks' wrapper quotes its specifiers with double quotes via
    // JSON.stringify, unlike claude-code-hooks' single quotes.)
    expect(shallowEntry).toContain(`import { execute } from "./node_modules/@goodfoot/codex-hooks/src/runtime`);
  }, 60000);
});
