 ...
 */
export const DEFAULT_WAKE_WORD = "Hey Computer";
...
// Server → browser outbound wire type.
// The controller's existing private BrowserEnvelope (inbound browser→server) stays in controller.ts and is not exported.
export type ServerEnvelope =
  ...
  | { type: "voice.session.token"; data: { clientSecret: string; model: string; expiresAt?: number } }
  | { type: "voice.session.close"; data: { code: number; reason: string } }
  | { type: "voice.send"; data: { event: XAIClientEvent; gate?: "playback-drained" } }
  | { type: "audio.output.delta"; data: { audio: string } }
