...
function unorderedArrayEquality(a: unknown, b: unknown): boolean | undefined {
  if (Array.isArray(a) && Array.isArray(b)) {
    if (a.length !== b.length) return false;

    const bCopy = Array.from(b) as unknown[];
    for (const itemA of a) {
      const index = bCopy.findIndex((itemB) => equals(itemA, itemB, [unorderedArrayEquality, iterableEquality]));
      if (index === -1) {
        return false;
      }
...
expect.extend({
  ...
  toEqualSorted(received: unknown, expected: unknown) {
    // Use equals from @jest/expect-utils with custom equality testers
    const pass = equals(received, expected, [unorderedArrayEquality, iterableEquality]);
