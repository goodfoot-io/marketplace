...
// If no explicit connection variables are set, postgres will try to connect to default localhost:5432
...
// Skip tests if no PostgreSQL connection variables are configured
const skipIfNoPostgres = !hasPostgresConfig;

describe('Test Utilities', () => {
  test.skipIf(skipIfNoPostgres)('Should make a connection to a test database', async () => {
    const { sql } = await getTestSql();
    await expect(sql`SELECT 1 as foo`).resolves.toEqual([{ foo: 1 }]);
  });

  test.skipIf(skipIfNoPostgres)('Should cleanup a test database', async () => {
    // destroySql is called on process.on('TEARDOWN', ...)
  ...
  test.skipIf(skipIfNoPostgres)(
    'Should destroy old connections if the number of connections exceeds MAX_CONCURRENT_CONNECTIONS',
    async () => {
