# Principle 4 — Multiple paths to the same topic

Scope: why every topic needs more than one path. Explanation mode.

A corpus rarely has one natural hierarchy. Readers seek by many frames — role, lifecycle stage, failure mode, API surface, component — and a single directory tree serves only one. The goal is enough high-scent paths for the ways readers actually seek, not every possible path.
