...
## 3. Add scope statement and a navigational index for long documents

Every document over 500 lines needs BOTH a scope statement at the top AND a navigational index; never add only one. A shorter document gets a scope statement only when it passes the value test (`strengthen-scent.md` §3) — a descriptive title needs no restatement.

The index is usually a table of contents. When a table of contents would not serve — a flat changelog or history with hundreds of version or date headings is the common case — build a curated index instead: a compact table mapping version or year ranges to their first heading (a Ctrl+F anchor), placed between the title and the first entry and set off by a horizontal rule. A 2,000-line changelog becomes skimmable; an auto-generated TOC of every version heading does not.
...
Whatever the directory holds, run `git ls-files <dir>` before writing and account for **every** significant file. A hub that silently omits files (a `.github/` hub that lists the policy docs but skips CODEOWNERS, the CI workflows, and the dependency config) reads as complete while it is not — worse than no hub. Group the entries when the directory mixes kinds (policy, templates, automation), and note any file left out on purpose (generated, tool-internal) with a one-line reason rather than dropping it silently.
