...
function isExecuteToolArguments(value) {
...
function validateExecuteToolArguments(value) {
...
function isToolUseContent(content) {
  return typeof content === "object" && content !== null && "type" in content && content.type === "tool_use" && "name" in content && typeof content.name === "string" && "input" in content && typeof content.input === "object";
}
function isToolResultContent(content) {
  return typeof content === "object" && content !== null && "type" in content && content.type === "tool_result" && "content" in content;
}
...
function isSdkUserMessage(message) {
  return typeof message === "object" && message !== null && "type" in message && message.type === "user";
}
function isSdkAssistantMessage(message) {
  return typeof message === "object" && message !== null && "type" in message && message.type === "assistant";
}
function getEnvironmentAsRecord(env) {
  const result = {};
...
async function findChromeMcpChildProcesses() {
  try {
...
async function killHungChromeMcpProcesses() {
  try {
...
var CHROME_SYSTEM_INSTRUCTIONS = String.raw`Browser automation agent using Chrome DevTools MCP server.
...
1. \`click(uid)\` \u2192 times out (expected)
2. \`handle_dialog(action="accept")\` \u2192 succeeds
3. \`take_snapshot()\` \u2192 fresh UIDs
...
- For other waits \u2192 use \`evaluate_script\` with async polling
...
**Exception**: \`fill_form()\` takes snapshot automatically
...
| Need | Tool |
|------|------|
| UIDs for interaction | \`take_snapshot()\` |
| Extract data | \`evaluate_script()\` |
| Visual verification | \`take_screenshot(filePath=...)\` |
| Fill multiple fields | \`fill_form()\` (preferred) |
| Fill single field | \`fill()\` |
| Wait for text | \`wait_for()\` |
| Wait for anything else | \`evaluate_script()\` with polling |
...
- Data \u2192 use \`evaluate_script\` not snapshot
- Network requests \u2192 paginate with \`pageSize\`
...
function parseChromeArgs() {
  try {
    const { values } = parseArgs({
      options: {
        browserUrl: { type: "string", short: "b" },
...
var cleanupIntervalId = setInterval(cleanupStaleSessions, 5 * 60 * 1e3);
...
function formatProgressMessage(toolName, input) {
...
async function logBrowserExecution(sessionId, prompt, transcript) {
...
server.setRequestHandler(ListToolsRequestSchema, () => ({
  tools: [
    {
      name: "prompt",
      description: "Automate browser tasks: navigate pages, interact with elements, capture screenshots, debug issues, and analyze performance",
      inputSchema: {
        type: "object",
        properties: {
          prompt: {
...
server.setRequestHandler(CallToolRequestSchema, async (request, meta) => {
  ...
  const progressToken = meta && typeof meta === "object" && "_meta" in meta && typeof meta._meta === "object" && meta._meta !== null ? meta._meta.progressToken : void 0;
...
- Respond with data only (no process description)`;
    ...
    const heartbeatCheckInterval = setInterval(() => {
      ...
      if (timeSinceLastHeartbeat > heartbeatTimeoutMs && !mcpProcessHung) {
        ...
        console.error(
          `[Heartbeat Watchdog] No progress for ${Math.floor(timeSinceLastHeartbeat / 1e3)}s - Chrome MCP may be hung`
        );
        console.error("[Heartbeat Watchdog] Attempting to kill hung Chrome MCP processes...");
        killHungChromeMcpProcesses().then((killedCount) => {
          if (killedCount > 0) {
                ...
                if (content.is_error) {
                  ...
                  if (errorContent.includes("No page selected") && !noPageSelectedDetected) {
                    ...
                    try {
                      ...
                      if (fetchError.message === "NO_PAGE_SELECTED_RECOVERED") {
                        throw fetchError;
                      }
        ...
        for await (const message of query({
          ...
          if (isSdkResultMessage(msg)) {
            ...
            if (userMsg && userMsg.content && Array.isArray(userMsg.content)) {
              for (const content of userMsg.content) {
                if (isToolResultContent(content)) {
                  const responseContent = content.is_error ? String(content.content) : typeof content.content === "string" ? content.content : JSON.stringify(content.content, null, 2);
                  transcript.push(
...
**Log**: ${logPath}`;
    ...
    clearTimeout(timeoutId);
    session.conversationHistory.push(`User: ${prompt}`);
    session.conversationHistory.push(`Error: ${error.message}`);
    if (error instanceof Error && error.message === "Operation was aborted") {
      const timeoutMinutes = Math.floor(timeoutMs / 6e4);
      throw new McpError(
        ErrorCode.InternalError,
        `Browser automation request timed out after ${timeoutMinutes} minutes. Session: ${sessionId}. This typically indicates: (1) Chrome became unresponsive or disconnected, (2) A page is stuck loading resources, or (3) Bot detection is blocking automation. Check the log for details: ${logPath || "not available yet"}`
      );
    }
    const errorMessage = `Browser automation failed: ${error.message}. Session: ${sessionId}, Tool calls executed: ${toolCallCount}. This may indicate a connection issue, element interaction failure, or page loading problem. Review the session log for diagnostic details.`;
