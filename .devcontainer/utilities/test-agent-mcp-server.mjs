  ...
  "../../node_modules/ajv/lib/compile/index.js"(exports, module) {
    ...
    function compile(schema2, root, localRefs, baseId) {
      ...
      function useCustomRule(rule, schema3, parentSchema, it) {
        ...
        var validate;
        if (compile2) {
          validate = compile2.call(self, schema3, parentSchema, it);
        } else if (macro) {
          validate = macro.call(self, schema3, parentSchema, it);
          if (opts.validateSchema !== false) self.validateSchema(validate, true);
        } else if (inline) {
          validate = inline.call(self, it, rule.keyword, schema3, parentSchema);
        } else {
...
function parseFrontMatter(content) {
  ...
  if (match) {
    try {
      const frontMatter = js_yaml_default.load(match[1]);
...
async function logAgentExecution(systemInstructionsFile, description, prompt, transcript) {
  ...
  const timestamp2 = now.toISOString().replace(/[:.]/g, "-").replace("T", "_").slice(0, -1);
  const filename = `${timestamp2}.md`;
  ...
  if (system_instructions_file) {
    ...
    try {
      ...
      throw new McpError(
        ...
        `Failed to read system instructions file: ${error.message}`
      );
    }
  }
  const abortController = new AbortController();
  ...
  try {
    ...
    if (frontMatter) {
      ...
      if (frontMatter.tools) {
        const toolsList = frontMatter.tools.split(",").map((t) => t.trim().replace(/^["']|["']$/g, "")).filter((t) => t.length > 0);
    ...
    for await (const message of query({
      ...
      console.error(`[DEBUG] Received message #${messageCount}:`, {
        type: message.type,
        subtype: "subtype" in message ? message.subtype : void 0
      });
      if (abortController.signal.aborted) {
