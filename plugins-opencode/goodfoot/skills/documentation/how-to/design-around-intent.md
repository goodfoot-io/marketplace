...
When two candidate changes compete for the same effort, prefer the **structural** one. A structural change alters how readers find and use information — an index over a long file, a cross-reference web, a routing hub, a reorganization. A cosmetic change alters how content looks without changing findability — a label, a restatement, a scope line on a page the title already orients. Structural moves return more per line changed; weight them first, and do not pad a pass with cosmetic additions that fail the value test (`strengthen-scent.md` §3).
