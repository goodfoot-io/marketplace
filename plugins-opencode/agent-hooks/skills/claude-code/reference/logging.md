...
```bash
export AGENT_HOOKS_LOG_FILE=/tmp/claude-hooks.log
```

### Build Flag (Hardcoded Path)

Best if you want to force logging for a specific build. The path is baked into the bundle, but a runtime `AGENT_HOOKS_LOG_FILE` env var will override it.

```bash
npx -y @goodfoot/agent-hooks --agent claude-code ... --log /tmp/claude-hooks.log
```
...
```bash
npx -y @goodfoot/agent-hooks --agent claude-code ... --log-env-var MY_PLUGIN_LOG_FILE
```
...
```typescript
import { Logger, type LogEvent } from '@goodfoot/agent-hooks/claude-code';
