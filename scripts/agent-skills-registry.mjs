...
const DEFAULT_REGISTRY = path.join(repo, "packages/plugin-layout-checks/registry/plugins.json");
...
export function assertNoUntrackedInTargets(registry) {
  for (const plugin of registry.plugins) {
    for (const target of plugin.targets) {
      const untracked = execFileSync("git", ["ls-files", "--others", "--exclude-standard", "--", target.path], {
        cwd: repo,
        encoding: "utf8",
      })
        .split("\n")
