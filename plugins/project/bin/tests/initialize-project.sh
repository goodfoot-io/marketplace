...
INIT_PROJECT="$SCRIPT_DIR/../initialize-project"
        ...
        if [ $exit_code -eq 0 ] && [ "$output" = "$expected_path" ]; then
            ...
            if [ -d "$expected_path" ]; then
                ...
                if [ -f "$expected_path/log.md" ]; then
                    ...
                    if grep -q "# Implementation Project: $project_name" "$expected_path/log.md" && \
                       grep -q "## Project Initialization" "$expected_path/log.md"; then
...
if [ -f "projects/new/log-test-project/log.md" ]; then
    ...
    has_header=$(echo "$log_content" | grep -c "# Implementation Project: log-test-project")
    has_section=$(echo "$log_content" | grep -c "## Project Initialization")

    if [ $has_header -eq 1 ] && [ $has_section -eq 1 ]; then
...
if [ "$output" = "$expected_path" ] && [ -d "$expected_path" ] && [ -f "$expected_path/log.md" ]; then
    ...
    if grep -q "# Implementation Project: icebox-test-project" "$expected_path/log.md"; then
...
if [ "$output" = "$expected_path" ] && [ -d "$expected_path" ] && [ -f "$expected_path/log.md" ]; then
    # Verify the new project was created
    if grep -q "# Implementation Project: complete-test-project" "$expected_path/log.md"; then
