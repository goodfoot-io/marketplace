...
if [ -x "$ACTIVATE_PROJECT_WITH_ARGS" ]; then
    ...
    output=$("$ACTIVATE_PROJECT_WITH_ARGS" "nonexistent-test-project-$$" 2>&1)
    ...
    output=$("$ACTIVATE_PROJECT_WITH_ARGS" '"nonexistent-test-project-$$", please' 2>&1)
    if [[ "$output" == *"Project not found"* ]] || [[ "$output" == *"unmet dependencies"* ]]; then
        echo -e "${GREEN}✓${NC}"
    else
        echo -e "${YELLOW}⚠ Unexpected output${NC}"
    ...
    output=$("$ACTIVATE_PROJECT_WITH_ARGS" "@nonexistent-test-project-$$ status" 2>&1)
    if [[ "$output" == *"Project not found"* ]] || [[ "$output" == *"unmet dependencies"* ]]; then
        echo -e "${GREEN}✓${NC}"
    else
        echo -e "${YELLOW}⚠ Unexpected output${NC}"
    ...
    if [ -d "$REPO_ROOT/projects/ready-for-review/expandable-events-voice-instructions" ] || \
       ...
       [ -d "$REPO_ROOT/projects/pending/expandable-events-voice-instructions" ]; then
        ...
        output=$("$ACTIVATE_PROJECT_WITH_ARGS" "expandable-events-voice-instructions." 2>&1)
