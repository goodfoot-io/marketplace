...
ACTIVATE_PROJECT_WITH_ARGS="$SCRIPT_DIR/../activate-project-with-args"

# Setup test environment
export REPO_ROOT="${REPO_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
TEST_DIR="/tmp/activate-project-with-args-test-$$"
...
test_successful_activation() {
    ...
    output=$("$ACTIVATE_PROJECT_WITH_ARGS" "test-project" 2>&1)
...
test_project_not_found() {
    ...
    output=$("$ACTIVATE_PROJECT_WITH_ARGS" "nonexistent" 2>&1)
...
test_no_arguments() {
    ...
    output=$("$ACTIVATE_PROJECT_WITH_ARGS" 2>&1)
...
EOF
    ...
    output=$("$ACTIVATE_PROJECT_WITH_ARGS" "fail-project" 2>&1)
...
test_project_with_issue_info() {
    ...
    output=$("$ACTIVATE_PROJECT_WITH_ARGS" "test-project tests are failing" 2>&1)
...
test_project_with_period() {
    ...
    output=$("$ACTIVATE_PROJECT_WITH_ARGS" "Continue with punctuation-project." 2>&1)
...
test_project_with_quotes() {
    ...
    output=$("$ACTIVATE_PROJECT_WITH_ARGS" 'Work on "punctuation-project", please!' 2>&1)
...
test_project_with_at_symbol() {
    ...
    output=$("$ACTIVATE_PROJECT_WITH_ARGS" "Check @punctuation-project status" 2>&1)
...
EOF
    ...
    output=$("$ACTIVATE_PROJECT_WITH_ARGS" "sync-bug-project" 2>&1)
