...
ACTIVATE_PROJECT="$SCRIPT_DIR/../activate-project"
...
run_test() {
    ...
    output=$($ACTIVATE_PROJECT "$project_path" 2>"$stderr_file")
    local exit_code=$?
...
$ACTIVATE_PROJECT "projects/pending/duplicate-test" >/dev/null 2>&1
...
output=$($ACTIVATE_PROJECT "projects/ready-for-review/duplicate-test" 2>&1)
exit_code=$?
if [ $exit_code -ne 0 ]; then
...
output=$($ACTIVATE_PROJECT "projects/pending/printf-test" 2>&1)
exit_code=$?
...
ACTIVE_PROJECT=$($ACTIVATE_PROJECT "$PROJECT_PATH" 2>/dev/null)
