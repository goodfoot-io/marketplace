...
| `CODEX_HOOKS_LOG_FILE` set | JSON lines appended to file |
...
Best for general debugging. The Logger reads `CODEX_HOOKS_LOG_FILE` automatically at construction time.

```bash
export CODEX_HOOKS_LOG_FILE=/tmp/codex-hooks.log
```
...
import { Logger, type PreToolUseInput } from '@goodfoot/codex-hooks';
...
```typescript
import { Logger, type LogEvent } from '@goodfoot/codex-hooks';
...
1. Did you set `CODEX_HOOKS_LOG_FILE`?
