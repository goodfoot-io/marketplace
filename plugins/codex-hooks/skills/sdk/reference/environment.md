...
`@goodfoot/codex-hooks` deliberately ships **no environment helpers**. Codex does not expose a Claude-equivalent of `$CLAUDE_ENV_FILE` or a `getProjectDir()` indirection — the same data is already present on every hook input.
...
import { sessionStartHook, sessionStartOutput, stopHook, stopOutput } from '@goodfoot/codex-hooks';
