...
description: Fix bugs using test-first methodology: create a minimal failing test, then resolve with verified fix. Use when creating reproduction tests, debugging with test-driven development, or ensuring fixes pass automated regression tests.
---

# Reproduce Then Resolve

Systematically fix bugs using test-first methodology: create a minimal failing test that demonstrates the bug, then implement a fix that makes the test pass.
...
1. Extract [BUG_DESCRIPTION] from recent conversation history
2. Extract [SCOPE_HINT] if mentioned
...
Increment [REPRODUCTION_ATTEMPT] (max 5)
...
## Code Exploration
ALWAYS read and understand relevant files before writing test code. Do not speculate about code you have not inspected. Be rigorous and persistent in searching code for key facts. Thoroughly review the style, conventions, and existing test patterns of the codebase before creating new tests.
...
Increment [RESOLVE_ATTEMPT] (max 5)
...
## Code Exploration
ALWAYS read and understand relevant source files before proposing fixes. Do not speculate about code you have not inspected. If the bug involves specific files, you MUST open and inspect them before implementing fixes. Be rigorous and persistent in searching code for key facts.
