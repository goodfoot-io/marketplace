---
description: Iteratively improve a slash command to match reference analysis
---

Test and iteratively refine a slash command's components through evidence-based modifications until it matches reference evaluation criteria.
...
<parameter name="prompt">Follow the instructions in @.claude/commands/utilities/map-integration-chain.md replacing $ARGUMENTS with: "[SLASH_COMMAND]"</parameter>
</invoke>
```
...
3.1. Review the most recent evaluation results.

3.2. If the user specified [EDITABLE_FILE_LIST]:
- 3.2.1. Review the role of each file in [INTEGRATION_CHAIN_FILE_LIST]
- 3.2.2. Think deeply. Create three draft theories on how to improve the slash command's performance according to [EVALUATION] by editing one or more files in [EDITABLE_FILE_LIST]. 
- 3.2.3. For each draft theory, identify 1-3 pieces of supporting evidence and 1-3 pieces of contradicting evidence.
- 3.2.4. Choose the best theory and make improvements to one or more of the files in [EDITABLE_FILE_LIST].

3.3. If the user did not specify [EDITABLE_FILE_LIST]:
- 3.3.1. Review the role of each file in [INTEGRATION_CHAIN_FILE_LIST]
- 3.3.2. Think deeply. Create three draft theories on how to improve the slash command's performance according to [EVALUATION] by editing one or more files in [INTEGRATION_CHAIN_FILE_LIST]. 
- 3.3.3. For each draft theory, identify 1-3 pieces of supporting evidence and 1-3 pieces of contradicting evidence.
- 3.3.4. Choose the best theory and make improvements to one or more of the files in [INTEGRATION_CHAIN_FILE_LIST].
...
6.2. If there have been fewer than ten iterations, return to Phase 3 and continue the iterative improvement process.
