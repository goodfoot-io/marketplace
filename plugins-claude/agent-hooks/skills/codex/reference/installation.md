...
```bash
# Scaffold into packages/, output hooks.json to plugins-claude/
npx @goodfoot/agent-hooks --agent codex --scaffold ./packages/my-codex-hooks \
  --hooks PreToolUse,PostToolUse \
  -o ../../plugins-claude/my-plugin/.codex/hooks.json
```

This generates the build script:
```json
"build": "npx -y @goodfoot/agent-hooks --agent codex -i \"src/**/*.ts\" -o \"../../plugins-claude/my-plugin/.codex/hooks.json\""
```
...
npm run build
# Check: plugins-claude/my-plugin/.codex/hooks.json should exist
cat ../../plugins-claude/my-plugin/.codex/hooks.json | jq .
```
