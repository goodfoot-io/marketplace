---
name: handbook
description: Troubleshooting and detailed usage for the voice MCP server —
  browser/audio problems, conversation lifecycle (pause/resume/reset), context
  management, intervening when the avatar is wrong, the full-viewport HTML
  stage, ending a session, and startup/channel failures. Load a reference below
  when a live voice conversation needs deeper handling than the start message
  covers, or when something is not working.
---
