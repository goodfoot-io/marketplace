...
A topic documented in one layer with no path from the others is an orphan across layers even when it has inbound links within its own; `validate.md` re-checks this.
