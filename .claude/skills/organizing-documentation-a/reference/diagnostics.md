# Diagnostics — start from a symptom

Scope: route from an observed problem to the topic that owns the fix. The situational entry into the skill (Principle 1; recovery, Principle 7). Pure routing — each symptom links its owner.

- **A directory holds three or more docs with no overview**: `reference/principles/03-hubs-orientation-and-routing/hub-necessity.md` — the three-or-more-docs threshold and the don't-build-an-empty-hub rule.
...
- **A cross-reference points at a volatile line number**: `reference/foundations.md` stable-identifier section — re-anchor to a SHA-pinned fragment link or mesh name.
- **Doc↔code drift is not mechanically detectable**: `reference/principles/08-validate-and-iterate/governance.md` — anchor claims and gate wiki check / git mesh stale in CI.
- **A load-bearing claim can't be anchored**: `reference/principles/08-validate-and-iterate/governance.md` — the gate never sees it; flag the owning code area's maintainers and lean on freshness/last-verified, not a doc owner.
