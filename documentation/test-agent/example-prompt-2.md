/test-agent-v11 `/workspace/.worktrees/discovery-command/.claude/agents/documenter.md` / documenter / Optimize the documenter agent to produce reports that serve as effective starting points for understanding codebases.
...
Spawn a `general-purpose` agent using the Task tool with:
...
After the general-purpose agent completes its response, use the `documenter-evaluator` agent to score the output using the Task tool:

```
Task(
  description="Evaluate general-purpose agent response",
  subagent_type="documenter-evaluator",
  prompt=`
    ## Evaluation Request
...
1. **Generate Documentation**:
   ...
   mcp__test-agent__task(
     description="Generate architectural documentation",
     system_instructions_file="/workspace/.worktrees/discovery-command/.claude/agents/documenter.md",
     prompt="Document packages/models"
   )
   ```

2. **Test with General-Purpose Agent**:
   ```
   Task(
     description="Test documentation with architectural question",
     subagent_type="general-purpose",
     prompt="[question with @documentation]"
   )
   ```

3. **Evaluate with documenter-evaluator**:
   ```
   Task(
     description="Evaluate general-purpose agent output",
     subagent_type="documenter-evaluator",
     prompt="[evaluation request with agent output]"
   )
   ```

4. **Record Results**:
   - Documenter Variation ID
   - Overall Score
   - Dimension Scores
   - Evidence Quality Metrics

5. **Repeat** for all variations and scenarios

6. **Analyze** to identify the optimal documenter configuration