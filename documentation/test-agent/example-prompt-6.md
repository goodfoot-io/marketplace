...
### Test Task
The documenter agent will generate architectural documentation for specified codebase directories or packages (NOT individual files). A general-purpose agent will then use this documentation to answer complex architectural questions. Success is measured by how effectively the documentation enables rapid, accurate understanding of multi-file systems.

### Input Format
Each test scenario will provide:
- A VERIFIED directory or package path (must exist - check with Bash/ls first)
- The instruction to create architectural documentation for the entire directory/package
- A complex architectural question about interactions between files in that directory
- **CRITICAL**: Only use directories/packages confirmed through tool outputs, never assume paths exist

### Expected Output
- Documenter: A comprehensive architectural report saved to `/workspace/reports/project-state/[component]-[TIMESTAMP].md`
...
### Step 0: Discover Real Directories and Packages (REQUIRED)

Before creating ANY test scenarios, discover actual directories and packages in the codebase:

<example-tool-use>
# List all packages in the monorepo
Bash(command="ls -la /workspace/.worktrees/discovery-command/packages")

# Find directories with substantial TypeScript content
Bash(command="find /workspace/.worktrees/discovery-command/packages -type d -maxdepth 2 | xargs -I{} sh -c 'echo -n \"{}: \"; find {} -name \"*.ts\" -type f 2>/dev/null | wc -l' | sort -t: -k2 -rn | head -20")

# List subdirectories in packages that contain source code
Bash(command="find /workspace/.worktrees/discovery-command/packages -type d -name 'src' -o -name 'lib' -o -name 'server' -o -name 'client' | head -20")

# Verify package.json files to identify actual packages
Glob(pattern="packages/*/package.json", path="/workspace/.worktrees/discovery-command")

# Check for TypeScript configuration in packages
Glob(pattern="packages/*/tsconfig.json", path="/workspace/.worktrees/discovery-command")
</example-tool-use>

**MANDATORY**: Store discovered directories/packages and use ONLY these verified paths in test scenarios. Never test individual files - always test directories or packages.

### Step 1: Generate Documentation
Execute the documenter agent on a VERIFIED directory or package:

<example-tool-use>
# First verify the directory exists and contains files
Bash(command="ls -la [proposed_directory_path] | head -10")
Bash(command="find [proposed_directory_path] -name '*.ts' -type f | wc -l")

# Only if successful, proceed with documentation generation
mcp__test-agent__task(
  description="Generate documentation for package/directory",
  prompt="Create architectural documentation for `[VERIFIED_DIRECTORY_PATH]`",
  system_instructions_file="[working_directory]/$AGENT_ID.md"
)
</example-tool-use>

Capture the documentation file path from the output (should be in format: reports/project-state/[name]-[timestamp].md).
...
mcp__test-agent__task(
  ...
  prompt=`
    ...
    "[CROSS_FILE_ARCHITECTURAL_QUESTION_ABOUT_DIRECTORY]"

    Example questions for directories/packages:
    - "How do the services in this package communicate with each other?"
    - "What is the dependency flow between modules in this directory?"
    - "How does error handling propagate through the components in this package?"
    - "What are the main interfaces between files in this directory?"
...
## Finding Suitable Test Directories and Packages

For directory/package-focused testing, use these patterns to discover real multi-file components:

<example-tool-use>
# Find packages with substantial TypeScript content
Bash(command="for dir in /workspace/.worktrees/discovery-command/packages/*; do echo -n \"$(basename $dir): \"; find $dir -name '*.ts' -type f 2>/dev/null | wc -l; done | sort -t: -k2 -rn")

# Find directories with multiple interconnected TypeScript files
Bash(command="find /workspace/.worktrees/discovery-command/packages -type d -path '*/src' -o -path '*/lib' | while read dir; do file_count=$(find \"$dir\" -name '*.ts' -type f 2>/dev/null | wc -l); if [ $file_count -gt 5 ]; then echo \"$dir: $file_count files\"; fi; done")

# Identify packages with complex internal structure
Bash(command="for pkg in /workspace/.worktrees/discovery-command/packages/*; do if [ -d \"$pkg/src\" ]; then echo \"Package: $(basename $pkg)\"; find \"$pkg/src\" -type d | head -5; fi; done")

# Find packages with cross-file imports
Grep(pattern="from ['\"]\\./", glob="packages/*/src/**/*.ts", output_mode="files_with_matches", head_limit=20)
</example-tool-use>

Select diverse directories/packages from the results:
- Choose packages with varying file counts (5-10 files, 10-20 files, 20+ files)
- Include packages with different architectural patterns (services, models, APIs)
- Ensure variety in directory structures (flat vs nested)
- Verify each selected directory contains multiple interconnected files

## Handling Missing or Invalid Paths

If a suggested directory doesn't exist or can't be accessed:

1. **Immediate verification**: `Bash(command="ls -la [directory_path]")`
2. **Check if it's a package**: `Bash(command="test -f [directory_path]/package.json && echo 'Package found' || echo 'Not a package'")`
3. **Count files in directory**: `Bash(command="find [directory_path] -name '*.ts' -type f | wc -l")`
4. **List subdirectories**: `Bash(command="find [directory_path] -type d -maxdepth 1")`
5. **Select alternative**: Choose a verified alternative directory/package that serves the same testing purpose

**NEVER** create scenarios with non-existent directories - always verify first and document the verification in your scenario.

## Test Scenario Generation Process

### REQUIRED: Path Verification for Each Scenario

Before creating each test scenario file:

<example-tool-use>
# 1. Propose a directory/package based on discovery
proposed_directory = "[directory_from_discovery_phase]"

# 2. Verify it exists and contains multiple files
Bash(command="ls -la [proposed_directory] | head -10")
Bash(command="find [proposed_directory] -name '*.ts' -type f | wc -l")
Bash(command="find [proposed_directory] -name '*.ts' -type f | head -5")

# 3. Only if successful (contains 5+ TypeScript files), create the scenario
Write(
  file_path="[working_directory]/test_scenario_[N].md",
  content="# Test Scenario [N]

## Verified Directory/Package Path
Path: [verified_directory]
Verification timestamp: [when_verified]
Directory exists: YES (verified via Bash command)
TypeScript file count: [number] files
Sample files: [list of 3-5 example files found]

## Test Scenario
Target: [Full directory/package path]
Question: [Cross-file architectural question about this directory]
..."
)
</example-tool-use>
...
Generate 5-10 test scenarios covering different aspects, but ONLY after discovering and verifying real paths:

### Discovery-Based Scenario Types
1. **Small Packages (5-10 files)** - Find packages with focused functionality
2. **Medium Packages (10-20 files)** - Discover packages with moderate complexity
3. **Large Packages (20+ files)** - Identify complex packages with multiple subdirectories
4. **Service Directories** - Find directories containing service/handler patterns
5. **Model/Type Directories** - Identify directories with data models and type definitions

### Dynamic Test Question Generation

Instead of hardcoded questions, generate questions based on discovered directories:

<example-tool-use>
# After discovering a directory, analyze its structure to create relevant questions
Bash(command="find [discovered_directory] -name '*.ts' -type f | head -10")
Grep(pattern="import.*from ['\"]\\./", glob="[discovered_directory]/**/*.ts", output_mode="content", head_limit=10)

# Based on the directory structure, generate appropriate questions like:
# - For service directories: "How do the services in [directory] coordinate to handle requests?"
# - For model directories: "What is the relationship between the data models in [directory]?"
# - For API directories: "How do the endpoints in [directory] share common middleware or validation?"
# - For mixed directories: "What is the architectural pattern that connects files in [directory]?"
</example-tool-use>
...
- **ALWAYS** verify directories exist and contain multiple files before using them in scenarios
- Use the current codebase (`/workspace/.worktrees/discovery-command`) as test material
- Test at least 3 different VERIFIED directories/packages for robustness
- Focus on multi-file architectural questions that require understanding file interactions
- Create diverse test questions that explore cross-file relationships and patterns
- Ensure scoring uses the full 0.0-1.0 range for discrimination
- Track which instruction patterns correlate with high scores for multi-file documentation
- Document specific examples of effective vs ineffective directory-level documentation
- All metrics must be independently calculable without shared state or baseline comparisons
- **Document directory verification** (file count, structure) in each test scenario for audit trail
- **Never test individual files** - always test directories or packages with 5+ files